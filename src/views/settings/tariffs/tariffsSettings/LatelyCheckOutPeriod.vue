<template>
  <div class="modal">
    <div class="modal-div-2">
      ...
      <div
        class="loop-div"
        v-for="(rule, idx) in checkOutRules"
        :key="idx"
      >
        <div class="form-2">
          <div class="w-100-timeSelect">
            <label for="input">Выезд С</label>
            <el-time-select
              class="time"
              v-model="rule.checkOutStartTime"
              disabled
              :picker-options="{
                start: '00:00',
                step: '00:01',
          ...
          <div class="w-100-timeSelect">
            <label for="input">Выезд ДО</label>
            <el-time-select
              class="time"
              v-model="rule.checkOutStopTime"
              :picker-options="{
                start: '00:00',
                step: '00:01',
                end: '23:59'
        ...
        >
          <button
            ...
            v-show="idx || ( !idx && checkOutRules.length > 1)"
          >
          ...
          <button
            class="ghost-btn"
            @click="addRule(idx)"
            v-show="idx == checkOutRules.length-1"
          >
...
export default {
  ...
  methods: {
    closeModal() {
      this.$emit('closeLateCheckOutPeriod')
    },

    addRule () {
      this.checkOutRules.push({
        checkOutStartTime: '',
        checkOutStopTime: '',
        tarificationType: 'fixed',
        amount: '',
    ...
    removeRule (idx) {
      this.checkOutRules.splice(idx, 1)
    },
  }
}
</script>
